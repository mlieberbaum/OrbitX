"""OrbitX Header Font P8: refined quadratic curves on P7 geometric outlines.

Build on GitHub Actions with no end-user Python installation. Retains the standard
TrueType tables and Windows compatibility approach of P5, and retains P7 spacing and M/N stroke weights, while replacing the deliberate
chamfers of rounded letters with tangent-continuous quadratic Bezier curves.
...
# a conventional lowercase alphabet. For consistency they inherit clean P8
...
# Only soften corners on characters intended to be rounded in the approved
# reference. Geometry and metrics stay EXACTLY as P7: for each existing diagonal
# chamfer connecting perpendicular horizontal/vertical strokes, replace the
# diagonal segment by a single quadratic Bezier. Its control point is the
# intersection of the continuing straight edges. Hence both ends are tangent
# to their neighbors, without smoothing intended angular strokes in A/M/N/X.
# Digits whose curved portions use P7 chamfers get the same treatment.
ROUND_GLYPHS = set('BCD G O P Q R S U J 0 2 3 5 6 8 9'.replace(' ', ''))

def smooth_chamfer_edges(contour):
    n = len(contour)
    smooth = {}
    for i in range(n):
        a, b, c, d = (contour[(i-1) % n], contour[i],
                      contour[(i+1) % n], contour[(i+2) % n])
        dx, dy = c[0]-b[0], c[1]-b[1]
        if dx == 0 or dy == 0:
            continue  # Not a diagonal chamfer.
        prev_h = a[1] == b[1] and a[0] != b[0]
        prev_v = a[0] == b[0] and a[1] != b[1]
        next_h = c[1] == d[1] and c[0] != d[0]
        next_v = c[0] == d[0] and c[1] != d[1]
        if prev_h and next_v:
            control = (c[0], b[1])
            if (b[0]-a[0]) * dx <= 0 or (d[1]-c[1]) * dy <= 0:
                continue
        elif prev_v and next_h:
            control = (b[0], c[1])
            if (b[1]-a[1]) * dy <= 0 or (d[0]-c[0]) * dx <= 0:
                continue
        else:
            continue
        # The control point lies at the intended right-angle corner, meaning
        # the quadratic preserves the P7 ink bounds and horizontal advance.
        smooth[i] = control
    return smooth
...
for ch, contours in {**paths, **P}.items():
    ...
    for contour_index, contour in enumerate(contours):
        ...
        def design_point(point):
            x, y = point
            return (round((x+shift_x)*scale),round(y*scale))
        pen.moveTo(design_point(contour[0]))
        is_rounded = ch.upper() in ROUND_GLYPHS
        rounded_edges = smooth_chamfer_edges(contour) if is_rounded else {}
        for i in range(len(contour)):
            j = (i+1) % len(contour)
            if i in rounded_edges:
                pen.qCurveTo(design_point(rounded_edges[i]),design_point(contour[j]))
            elif j != 0:
                pen.lineTo(design_point(contour[j]))
        pen.closePath()
...
family = 'OrbitX Header Font P8'
names = {1:family, 2:'Regular', 3:'OrbitX-Header-P8-Regular-20260920',
         4:family, 6:'OrbitXHeaderFontP8-Regular',16:family,17:'Regular'}
for record in font['name'].names:
    if record.nameID in names:
        value = names[record.nameID]
        record.string = value.encode('utf_16_be') if record.isUnicode() else value.encode('latin-1')
font['head'].fontRevision=0.8
...
out=HERE/'OrbitX_Header_Font_Prototype_8.ttf'
font.save(out)
font.close()
check=TTFont(out)
...
check.close()
print(f'P8 smooth-curve font generated: {out.name}')
